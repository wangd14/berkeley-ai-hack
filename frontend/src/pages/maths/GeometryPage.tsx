...
function getGeometryLessonPlan(language: string): any[] {
  ...
  const geometry = mathCourse.subcourses.find((s: any) => s.name === (language === 'Shona' ? 'Geometry' : 'Geometry'));
  if (!geometry || !geometry.lessonPlan) return [];
  return geometry.lessonPlan;
}

function getGeometrySubcourseName(language: string) {
  return language === 'Shona' ? 'Geometry' : 'Geometry';
}

export function GeometryPage() {
  const { language } = useLanguage();
  const lessonPlan: any[] = getGeometryLessonPlan(language);
    ...
    if (student_id) {
      fetch('/api/course-stats/complete-question', {
        ...
        body: JSON.stringify({
          ...
          subcourse: getGeometrySubcourseName(language),
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="flex flex-col space-y-4">
                <div className="bg-blue-50 rounded-xl p-4 flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="font-medium text-blue-900 text-lg">Points, Lines & Angles</h3>
                    <p className="text-blue-800">
                      The basic building blocks of geometry: points, lines, and angles form the foundation for all shapes.
                    </p>
                  </div>
                </div>
                <div className="bg-purple-50 rounded-xl p-4 flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="font-medium text-purple-900 text-lg">Shapes & Properties</h3>
                    <p className="text-purple-800">
                      Triangles, quadrilaterals, and circles have unique properties and rules in geometry.
                    </p>
                  </div>
                </div>
              </div>
