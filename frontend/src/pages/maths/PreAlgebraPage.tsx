...
function getPreAlgebraLessonPlan(language: string): any[] {
  ...
  const preAlgebra = mathCourse.subcourses.find((s: any) => s.name === (language === 'Shona' ? 'Pre-Algebra' : 'Pre-Algebra'));
  if (!preAlgebra || !preAlgebra.lessonPlan) return [];
  return preAlgebra.lessonPlan;
}

function getPreAlgebraSubcourseName(language: string) {
  return language === 'Shona' ? 'Pre-Algebra' : 'Pre-Algebra';
}

export function PreAlgebraPage() {
  const { language } = useLanguage();
  const lessonPlan: any[] = getPreAlgebraLessonPlan(language);
    ...
    if (student_id) {
      fetch('/api/course-stats/complete-question', {
        ...
        body: JSON.stringify({
          ...
          subcourse: getPreAlgebraSubcourseName(language),
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="flex flex-col space-y-4">
                <div className="bg-blue-50 rounded-xl p-4 flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="font-medium text-blue-900 text-lg">Integers & Rational Numbers</h3>
                    <p className="text-blue-800">
                      Integers are whole numbers and their opposites. Rational numbers can be written as fractions.
                    </p>
                  </div>
                </div>
                <div className="bg-purple-50 rounded-xl p-4 flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="font-medium text-purple-900 text-lg">Exponents & Radicals</h3>
                    <p className="text-purple-800">
                      Exponents represent repeated multiplication. Radicals are the opposite, like square roots.
                    </p>
                  </div>
                </div>
              </div>
