...
function getCourseSections(courseName: string, language: string) {
  ...
  const course = data.courses
    ? data.courses.find((c: any) => c.name === (language === 'Shona' ? 'Sainzi' : courseName))
    : null;
...
export function SciencePage() {
  ...
  const courseSections = getCourseSections('Science', language);
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-blue-50 to-purple-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">{language === 'Shona' ? 'Kosi yeSainzi' : 'Science Course'}</h1>
          <p className="text-gray-600 mt-1">{language === 'Shona' ? 'Dzidza sainzi nhanho nenhanho' : 'Explore science fundamentals step by step'}</p>
        </div>
