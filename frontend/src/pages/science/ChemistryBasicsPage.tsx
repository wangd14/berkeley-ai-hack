...
const ChemistryBasicsPage: React.FC = () => {
  ...
  // Find Chemistry Basics course data
  const scienceCourse = coursesData.courses.find((course: Course) => course.name === 'Science');
  const chemistryCourse = scienceCourse?.subcourses?.find((sub: any) => sub.name === 'Chemistry Basics');
  const lessonPlan = (chemistryCourse as any)?.lessonPlan || [];
